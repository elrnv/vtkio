...
impl<BO: ByteOrder> VtkParser<BO> {
    ...
    fn cells<'a>(input: &'a [u8], tag: &'static str, ft: FileType) -> IResult<&'a [u8], Cells> {
        do_parse!(input,
                  ...
                  (Cells { num_cells: n, vertices: data })
                 )
    }
    ...
    fn attribute_data(input: &[u8], n: usize, data_type: DataType, ft: FileType) -> IResult<&[u8], IOBuffer> {
        match data_type {
    ...
    fn attribute_scalars(input: &[u8], num_elements: usize, ft: FileType) -> IResult<&[u8], (String, Attribute)> {
        ws!( input, do_parse!(
                tag_no_case!("SCALARS") >>
                ...
                ((String::from(name), Attribute::Scalars {
                    ...
                    lookup_table: lookup_tbl_name
                        .and_then(|x| if x == "default" { None } else { Some(String::from(x)) }),
                    data
                }))
                ))
    }
    ...
    /// scalars.
    fn attribute_color_scalars_data(input: &[u8], n: usize, ft: FileType)
        -> IResult<&[u8], IOBuffer>
    {
        match ft {
    ...
    fn attribute_color_scalars(input: &[u8], num_elements: usize, ft: FileType)
    ...
    fn attribute_vectors(input: &[u8], num_elements: usize, ft: FileType) -> IResult<&[u8], (String, Attribute)> {
        ws!( input, do_parse!(
                   tag_no_case!("VECTORS") >>
                   name: map_res!( name, str::from_utf8 ) >>
                   dt: data_type >>
    ...
    fn attribute_normals(input: &[u8], num_elements: usize, ft: FileType)
        -> IResult<&[u8], (String, Attribute)>
    {
        ws!( input, do_parse!(
                   tag_no_case!("NORMALS") >>
    ...
    fn attribute_tex_coords(input: &[u8], num_elements: usize, ft: FileType)
        -> IResult<&[u8], (String, Attribute)>
    {
        ws!( input, do_parse!(
    ...
    fn attribute_tensors(input: &[u8], num_elements: usize, ft: FileType)
        -> IResult<&[u8], (String, Attribute)>
    {
        ws!( input, do_parse!(
                   tag_no_case!("TENSORS") >>
    ...
    fn attribute_field(input: &[u8], ft: FileType) -> IResult<&[u8], (String, Attribute)> {
        ws!( input, do_parse!(
                   ...
                   name: map_res!( name, str::from_utf8 ) >>
                   n: u32_b >>
                   data_array: many_m_n!( n as usize, n as usize, call!( Self::attribute_field_array, ft ) ) >>
                   (String::from(name), Attribute::Field { data_array })
                   ))
    }

    fn attribute(input: &[u8], num_elements: usize, ft: FileType) -> IResult<&[u8], (String, Attribute)> {
        ws!(  input,
             alt!(
    ...
    fn unstructured_grid(input: &[u8], ft: FileType) -> IResult<&[u8], DataSet> {
        ws!(input, do_parse!(
                ...
                data: call!( Self::attributes, ft ) >>
                (DataSet::UnstructuredGrid { points: p, cells: c, cell_types: ct, data })
                ))
    }
    ...
    fn poly_data_topo(input: &[u8], ft: FileType) -> IResult<&[u8], PolyDataTopology> {
           alt!(input,
               map!( call!( Self::cells, "LINES", ft ), |x| PolyDataTopology::Lines(x) ) |
               map!( call!( Self::cells, "POLYGONS", ft ), |x| PolyDataTopology::Polygons(x) )  |
               map!( call!( Self::cells, "VERTICES", ft ), |x| PolyDataTopology::Vertices(x) ) |
               map!( call!( Self::cells, "TRIANGLE_STRIPS", ft ), |x| PolyDataTopology::TriangleStrips(x))
               )
    }
    ...
    fn dataset(input: &[u8], file_type: FileType) -> IResult<&[u8], DataSet> {
           alt_complete!(
               ...
               do_parse!(
                   tag_no_case!("DATASET") >>
                   whitespace >>
                   tn: alt!( call!( Self::poly_data, file_type ) |
    ...
    fn vtk(input: &[u8]) -> IResult<&[u8], Vtk> {
        complete!( input, ws!( do_parse!(
                 h: header >>
                 d: call!( Self::dataset, h.2 ) >>
                 (Vtk { version: h.0, title: h.1, data: d })
                 )
           ) )
    }
}
...
mod tests {
    ...
    #[test] fn points_test() {
        ...
        assert_eq!(f, IResult::Done("other".as_bytes(),
    ...
    #[test] fn cells_test() {
        ...
        assert_eq!(f, IResult::Done("".as_bytes(), Cells { num_cells: 0, vertices: vec![] }));
        let f = <VtkParser>::cells(in2.as_bytes(), "CELLS", FileType::ASCII);
        assert_eq!(f, IResult::Done("other".as_bytes(), Cells { num_cells: 1, vertices: vec![2, 1, 2] }));
    }
    #[test] fn cell_type_test() {
    ...
    #[test] fn unstructured_grid_test() {
