...
mod write_vtk_impl {
    ...
    pub trait WriteVtkImpl {
        ...
        fn write_attrib(&mut self, data: Attributes, num_points: usize, num_cells: usize) {
            write!(self, "POINT_DATA {}\n", num_points).expect("Error writing point data header.");
            ...
            for (name, attrib) in data {
                match attrib {
                    ...
                    Attribute::TextureCoordinates { dim, data } => {
                        ...
                        self.write_buf(data, "Error writing TextureCoordinates attribute data.");
                    }
    ...
    impl WriteVtkImpl for Vec<u8> {
        ...
        fn write_cell_types(&mut self, data: Vec<CellType>, err: &str) {
            for t in data {
                self.write_i32::<NativeEndian>(t.to_i32().expect(err))
                    .expect(err);
            }
            write!(self, "\n").expect(err);
        }
        ...
        match vtk.data {
            DataSet::Field { name, data_array } => {
                ...
                for field in data_array {
                    ...
                    self.write_buf(field.data, "Error writing a new line after binary data.");
                }
            }
            ...
            DataSet::PolyData { points, topo, data } => {
                ...
                for topo_type in topo {
                    ...
                    self.write_u32_vec(cells.vertices, "Error writing PolyData topology data.");
            ...
            DataSet::UnstructuredGrid {
                ...
                self.write_u32_vec(cells.vertices, "Error writing UnstructuredGrid cell data.");
            ...
            DataSet::RectilinearGrid {
                ...
                self.write_buf(x_coords, "Error writing RectilinearGrid x_coordinates.");
                write!(
                    self,
                    "Y_COORDINATES {} {}\n",
                    y_coords.len(),
                ...
                self.write_buf(y_coords, "Error writing RectilinearGrid y_coordinates.");
                write!(
                    self,
                    "Z_COORDINATES {} {}\n",
                    z_coords.len(),
                ...
                self.write_buf(z_coords, "Error writing RectilinearGrid z_coordinates.");
