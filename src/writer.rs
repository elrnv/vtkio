...
mod write_vtk_impl {
    ...
    pub trait WriteVtkImpl {
        ...
        fn write_cell_types(&mut self, data: Vec<CellType>, err: &str);
        fn write_u32_vec(&mut self, data: Vec<u32>, err: &str);
        fn write_buf(&mut self, data: IOBuffer, err: &str);

        fn write_attrib(&mut self,
                        data: Attributes,
                        num_points: usize,
                        num_cells: usize)
        {
            write!(self, "POINT_DATA {}\n", num_points)
                .expect("Error writing point data header.");
            self.write_attrib_data(data.point);

            write!(self, "CELL_DATA {}\n", num_cells)
                .expect("Error writing cell data header.");
            self.write_attrib_data(data.cell);
        }

        fn write_attrib_data(&mut self, data: Vec<(String, Attribute)>) {
            for (name, attrib) in data {
                match attrib {
                    Attribute::Scalars { num_comp, lookup_table, data } => {
                        write!(self, "SCALARS {} {} {}\n",
                               name,
                    ...
                    Attribute::Tensors { data } => {
                        ...
                        self.write_buf(data, "Error writing Tensors attribute data.");
                    },
                    Attribute::Field { data_array } => {
                        ...
                        for field in data_array {
                            ...
                            self.write_buf(field.data, "Error writing Field attribute array data.");
                        }
                    },
                }
            }
    ...
    impl WriteVtkImpl for Vec<u8> {
        ...
        fn write_cell_types(&mut self, data: Vec<CellType>, err: &str) {
            for t in data {
                self.write_i32::<NativeEndian>(t.to_i32().expect(err)).expect(err);
            }
            write!(self, "\n").expect(err);
        }
        fn write_u32_vec(&mut self, data: Vec<u32>, err: &str) {
            let buf = IOBuffer::from(data);
            self.append(&mut buf.reinterpret_as_vec());
            write!(self, "\n").expect(err); // finish with a new line
        }
        fn write_buf(&mut self, data: IOBuffer, _err: &str) {
            self.append(&mut data.reinterpret_as_vec());
    ...
    impl WriteVtkImpl for String {
        ...
        fn write_u32_vec(&mut self, data: Vec<u32>, err: &str) {
            for i in 0..data.len() {
                ...
                if i < data.len()-1 { // add an extra space between elements
...
pub trait WriteVtk : write_vtk_impl::WriteVtkImpl {
    fn write_vtk(&mut self, vtk: Vtk) -> &mut Self {
        ...
        write!(self, "{}\n", vtk.title)
            .expect("Error writing VTK Title."); 
        ...
        match vtk.data {
            ...
            DataSet::PolyData { points, topo, data } => {
                ...
                self.write_buf(points, "Error writing PolyData points.");

                let mut num_cells = 0;
                for topo_type in topo {
                    match topo_type {
                        PolyDataTopology::Vertices(_) =>
                            write!(self, "VERTICES"),
                        PolyDataTopology::Lines(_) =>
                            write!(self, "LINES"),
                        PolyDataTopology::Polygons(_) =>
                            write!(self, "POLYGONS"),
                    ...
                    self.write_u32_vec(cells.vertices,
                                       "Error writing PolyData topology data.");

                    num_cells += cells.num_cells as usize;
                }

                self.write_attrib(data, num_points, num_cells);
            },

            DataSet::UnstructuredGrid { points, cells, cell_types, data } => {
                ...
                let num_points = points.len()/3;
                self.write_buf(points, "Error writing UnstructuredGrid points.");
                ...
                write!(self, "CELL_TYPES {}\n",
                       cell_types.len())
                    .expect("Error writing UnstructuredGrid cell types size.");

                let err_cell_types = "Error writing UnstructuredGrid cell types.";
                self.write_cell_types(cell_types, err_cell_types);

                self.write_attrib(data, num_points, cells.num_cells as usize);
            },

            DataSet::StructuredPoints { dims, origin, spacing, data } => {
                ...
                self.write_attrib(data, num_points, 0);
            },
            ...
            DataSet::StructuredGrid { dims, points, data } => {
                ...
                let num_points = points.len()/3;
                self.write_buf(points, "Error writing StructuredGrid points.");

                assert_eq!((dims[0]*dims[1]*dims[2]) as usize, num_points);
                self.write_attrib(data, num_points, 1);
            },

            DataSet::RectilinearGrid { dims, x_coords, y_coords, z_coords, data } => {
